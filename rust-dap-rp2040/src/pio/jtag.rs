    ...
    Srst: PinId,
{
    pub fn new(
        ...
        let tck_pin_id = tck.id().num; // Tck::DYN.num;
        let tms_pin_id = tms.id().num; // Tms::DYN.num;
        let tdi_pin_id = tdi.id().num; // Tdi::DYN.num;
        let tdo_pin_id = tdo.id().num; // Tdo::DYN.num;
        let trst_pin_id = if let Some(trst) = trst {
            Some(trst.id().num) // Trst::DYN.num)
        } else {
            None
        };
        let srst_pin_id: Option<u8> = if let Some(srst) = srst {
            Some(srst.id().num) // Srst::DYN.num)
        } else {
            None
        };
