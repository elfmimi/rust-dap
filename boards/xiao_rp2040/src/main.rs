...
mod app {
    ...
    fn uart_irq(mut c: uart_irq::Context) {
        ...
        while c.local.uart_rx_producer.ready() {
            let mut data = [Default::default(); 1];
            if let Ok(_) = c
                .shared
                .uart_reader
                .lock(|uart| uart.as_mut().unwrap().0.read(&mut data))
            {
                c.local.debug_out.toggle().ok();
                let _ = c.local.uart_rx_producer.enqueue(data[0]).ok(); // Enqueuing must not fail because we have already checked that the queue is ready to enqueue.
            } else {
                break;
            }
        }
